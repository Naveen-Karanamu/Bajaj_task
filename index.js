...
app.post("/bfhl", (req, res) => {
  ...
  if (req.body && Array.isArray(req.body.data) && req.body.data.length > 0) {
    ...
    const alphabets = inputData.filter(
      (item) =>
        typeof item === "string" && item.length === 1 && item.match(/[a-zA-Z]/)
    );
    ...
    const full_name = req.body.full_name.replace(/\s+/g, "_"); // Replace spaces with underscores
...
app.listen(PORT, () =>
  ConnectDB()
    ...
    .catch((error) =>
      console.log(error, "Server is running, But database connection failed!")
    )
);
